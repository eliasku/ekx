...
void setup_res_sprite(void) {
    ...
    {
        sprite_t* spr = &res_sprite.data[R_SPRITE_EMPTY];
...
uint16_t nine_patch_indices[54] = {
        0, 1, 5, 5, 4, 0,
        1, 1 + 1, 1 + 5, 1 + 5, 1 + 4, 1,
        2, 2 + 1, 2 + 5, 2 + 5, 2 + 4, 2,
        4, 4 + 1, 4 + 5, 4 + 5, 4 + 4, 4,
...
void draw_sprite(const sprite_t* sprite) {
    draw_sprite_rc(sprite, sprite->rect);
}

void draw_sprite_rc(const sprite_t* sprite, const rect_t rc) {
    if (!sprite->image_id) {
        return;
    }
    const sg_image image = REF_RESOLVE(res_image, sprite->image_id);
...
void draw_sprite_grid(const sprite_t* sprite, const rect_t grid, const rect_t target) {
    if (!sprite->image_id) {
        return;
    }
    const sg_image image = REF_RESOLVE(res_image, sprite->image_id);
    ...
    color_t cm = canvas.color[0].scale;
    color_t co = canvas.color[0].offset;
    /////
...
bool hit_test_sprite(const sprite_t* sprite, const vec2_t position) {
    (void) position;
    const sg_image image = REF_RESOLVE(res_image, sprite->image_id);
    if (image.id) {
        return true;
