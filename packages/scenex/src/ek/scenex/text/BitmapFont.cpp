...
bool BitmapFont::getGlyph(uint32_t codepoint, glyph_t* outGlyph) {
...
bool BitmapFont::getGlyphMetrics(uint32_t codepoint, glyph_t* outGlyph) {
    const uint32_t index = map.get(codepoint, 0xFFFFFFFFu);
    if (index != 0xFFFFFFFFu) {
        bmfont_glyph_t* glyph = file.glyphs + index;
        outGlyph->advanceWidth = glyph->advance_x;
        outGlyph->lineHeight = lineHeightMultiplier;
