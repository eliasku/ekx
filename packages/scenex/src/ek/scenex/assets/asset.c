...
static asset_type pack_asset;
static asset_type audio_asset;
static asset_type atlas_asset;
static asset_type dynamic_atlas_asset;
static asset_type scene_asset;
static asset_type bmfont_asset;
static asset_type ttf_asset;
static asset_type image_asset;
static asset_type strings_asset;
static asset_type model_asset;
...
static void image_do_load(asset_t* base) {
    image_asset_t* asset = (image_asset_t*) base;
...
static void image_poll(asset_t* base) {
    image_asset_t* asset = (image_asset_t*) base;
    if (asset->loader) {
...
static float image_progress(asset_t* base) {
    image_asset_t* asset = (image_asset_t*) base;
    if (base->state == ASSET_STATE_LOADING) {
...
static void image_do_unload(asset_t* base) {
    image_asset_t* asset = (image_asset_t*) base;
    if (asset->res) {
...
void init_asset_types(void) {
    pack_asset.load = base_load;
    pack_asset.unload = base_unload;
    ...
    pack_asset.type_size = sizeof(pack_asset_t);

    audio_asset.load = base_load;
    audio_asset.unload = base_unload;
    audio_asset.progress = base_progress;
    audio_asset.poll = audio_poll;
    audio_asset.do_load = audio_do_load;
    audio_asset.do_unload = audio_do_unload;
    audio_asset.type_size = sizeof(audio_asset_t);

    atlas_asset.load = atlas_load_;
    atlas_asset.unload = base_unload;
    atlas_asset.progress = atlas_progress;
    atlas_asset.poll = atlas_poll;
    atlas_asset.do_load = NULL;
    atlas_asset.do_unload = atlas_do_unload;
    atlas_asset.type_size = sizeof(atlas_asset_t);

    dynamic_atlas_asset.load = base_load;
    dynamic_atlas_asset.unload = base_unload;
    dynamic_atlas_asset.progress = base_progress;
    dynamic_atlas_asset.do_load = dynamic_atlas_do_load;
    dynamic_atlas_asset.do_unload = dynamic_atlas_do_unload;
    dynamic_atlas_asset.type_size = sizeof(dynamic_atlas_asset_t);

    scene_asset.load = base_load;
    scene_asset.unload = base_unload;
    scene_asset.progress = base_progress;
    scene_asset.do_load = scene_do_load;
    scene_asset.do_unload = scene_do_unload;
    scene_asset.type_size = sizeof(scene_asset_t);

    bmfont_asset.load = base_load;
    bmfont_asset.unload = base_unload;
    bmfont_asset.progress = base_progress;
    bmfont_asset.do_load = bmfont_do_load;
    bmfont_asset.do_unload = bmfont_do_unload;
    bmfont_asset.type_size = sizeof(bmfont_asset_t);

    ttf_asset.load = base_load;
    ttf_asset.unload = base_unload;
    ttf_asset.progress = base_progress;
    ttf_asset.do_load = ttf_do_load;
    ttf_asset.do_unload = ttf_do_unload;
    ttf_asset.type_size = sizeof(ttf_asset_t);

    image_asset.load = base_load;
    image_asset.unload = base_unload;
    image_asset.progress = image_progress;
    image_asset.poll = image_poll;
    image_asset.do_load = image_do_load;
    image_asset.do_unload = image_do_unload;
    image_asset.type_size = sizeof(image_asset_t);

    strings_asset.load = base_load;
    strings_asset.unload = base_unload;
    strings_asset.progress = base_progress;
    strings_asset.do_load = strings_do_load;
    strings_asset.do_unload = strings_do_unload;
    strings_asset.type_size = sizeof(strings_asset_t);

    model_asset.load = base_load;
    model_asset.unload = base_unload;
    model_asset.progress = base_progress;
    model_asset.do_load = model_do_load;
    model_asset.do_unload = model_do_unload;
    model_asset.type_size = sizeof(model_asset_t);
}
