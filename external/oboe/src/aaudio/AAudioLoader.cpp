...
AAudioLoader::~AAudioLoader() {
    ...
    OBOE_LOGI("%s() dlclose(%s) not called, OK", __func__, LIB_AAUDIO_NAME);
}
...
int AAudioLoader::open() {
    ...
    if (mLibHandle == nullptr) {
        OBOE_LOGI("AAudioLoader::open() could not find " LIB_AAUDIO_NAME);
        return -1; // TODO review return code
    } else {
        OBOE_LOGD("AAudioLoader():  dlopen(%s) returned %p", LIB_AAUDIO_NAME, mLibHandle);
    }
...
static void AAudioLoader_check(void *proc, const char *functionName) {
    if (proc == nullptr) {
        OBOE_LOGW("AAudioLoader could not find %s", functionName);
    }
}
