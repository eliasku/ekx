...
static void oboe_aaudio_error_thread_proc(AudioStreamAAudio *oboeStream,
                                          Result error) {
    OBOE_LOGD("%s(,%d) - entering >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", __func__, error);
...
void AudioStreamAAudio::internalErrorCallback(
    ...
    if (getSdkVersion() == __ANDROID_API_R__ && oboeResult == oboe::Result::ErrorTimeout) {
        oboeResult = oboe::Result::ErrorDisconnected;
        OBOE_LOGD("%s() ErrorTimeout changed to ErrorDisconnected to fix b/173928197", __func__);
    }
    ...
    if (oboeStream->wasErrorCallbackCalled()) { // block extra error callbacks
        OBOE_LOGE("%s() multiple error callbacks called!", __func__);
    } else if (stream != oboeStream->getUnderlyingStream()) {
        OBOE_LOGW("%s() stream already closed or closing", __func__); // might happen if there are bugs
...
void AudioStreamAAudio::logUnsupportedAttributes() {
    ...
    if (sdkVersion < __ANDROID_API_P__) {
        if (mUsage != Usage::Media) {
            OBOE_LOGW("Usage [AudioStreamBuilder::setUsage()] "
...
Result AudioStreamAAudio::open() {
    ...
    if (mDirection == oboe::Direction::Input
        ...
        capacity = kCapacityRequiredForFastLegacyTrack;
        OBOE_LOGD("AudioStreamAAudio.open() capacity changed from %d to %d for lower latency",
    ...
    if (mLibLoader->builder_setInputPreset != nullptr) {
        ...
        if (getSdkVersion() <= __ANDROID_API_P__ && inputPreset == InputPreset::VoicePerformance) {
            OBOE_LOGD("InputPreset::VoicePerformance not supported before Q. Using VoiceRecognition.");
    ...
    if (result != Result::OK) {
        ...
        if (result == Result::ErrorInternal && mDirection == Direction::Input) {
            OBOE_LOGW("AudioStreamAAudio.open() may have failed due to lack of "
    ...
    OBOE_LOGD("AudioStreamAAudio.open() format=%d, sampleRate=%d, capacity = %d",
...
error2:
    ...
    OBOE_LOGD("AudioStreamAAudio.open: AAudioStream_Open() returned %s",
...
DataCallbackResult AudioStreamAAudio::callOnAudioReady(AAudioStream * /*stream*/,
    ...
    if (result == DataCallbackResult::Continue) {
        ...
        if (result == DataCallbackResult::Stop) {
            OBOE_LOGD("Oboe callback returned DataCallbackResult::Stop");
        } else {
            OBOE_LOGE("Oboe callback returned unexpected value = %d", result);
        }
