...
Result DataConversionFlowGraph::configure(AudioStream *sourceStream, AudioStream *sinkStream) {
    ...
    OBOE_LOGI("%s() flowgraph converts channels: %d to %d, format: %d to %d"
    ...
    if ((isDataCallbackSpecified && isOutput)
        ...
        switch (sourceFormat) {
            ...
            case AudioFormat::I32:
                ...
                break;
            default:
                OBOE_LOGE("%s() Unsupported source caller format = %d", __func__, sourceFormat);
        ...
        switch (sourceFormat) {
            ...
            case AudioFormat::I32:
                ...
                break;
            default:
                OBOE_LOGE("%s() Unsupported source format = %d", __func__, sourceFormat);
                return Result::ErrorIllegalArgument;
        }
        if (isInput) {
    ...
    switch (sinkFormat) {
        ...
        case AudioFormat::I32:
            ...
            break;
        default:
            OBOE_LOGE("%s() Unsupported sink format = %d", __func__, sinkFormat);
            return Result::ErrorIllegalArgument;;
    }
