...
AudioStream *AudioStreamBuilder::build() {
    ...
    if (isAAudioRecommended() && mAudioApi != AudioApi::OpenSLES) {
        ...
        stream = new AudioStreamAAudio(*this);
        OBOE_LOGE("Creating AAudio stream on 8.0 because it was specified. This is error prone.");
    } else {
...
Result AudioStreamBuilder::openStream(AudioStream **streamPP) {
    ...
    if (result != Result::OK) {
        OBOE_LOGW("%s() invalid config %d", __func__, result);
        return result;
    }
    ...
    *streamPP = nullptr;
    ...
    if (conversionNeeded) {
        ...
        if (isCompatible(*tempStream)) {
            ...
            *streamPP = tempStream;
            ...
            OBOE_LOGI("%s() create a FilterAudioStream for data conversion.", __func__);
    ...
    if (result == Result::OK) {
        ...
        if (optimalBufferSize >= 0) {
            ...
            if (!setBufferResult) {
                OBOE_LOGW("Failed to setBufferSizeInFrames(%d). Error was %i",
                     optimalBufferSize,
                     setBufferResult.error());
            }
        }
