...
public:
    ...
    bool isMMapSafe(const AudioStreamBuilder &builder) override {
        ...
        if (wouldRecordSilence){
            OBOE_LOGI("QuirksManager::%s() Requested stream configuration would result in silence on "
...
bool QuirksManager::isConversionNeeded(
    ...
    if (isFloat
        ...
        conversionNeeded = true;
        OBOE_LOGI("QuirksManager::%s() forcing internal format to I16 for low latency", __func__);
    }
    ...
    if (isFloat
        ...
        conversionNeeded = true;
        OBOE_LOGI("QuirksManager::%s() float was requested but not supported on pre-L devices, "
             "creating an underlying I16 stream and using format conversion to provide a float "
             "stream", __func__);
    }

    // Channel Count conversions
    if (builder.isChannelConversionAllowed()
            && builder.getChannelCount() == kChannelCountStereo
            && isInput
            && isLowLatency
        ...
        conversionNeeded = true;
        OBOE_LOGI("QuirksManager::%s() using mono internally for low latency on O", __func__);
    } else if (builder.getChannelCount() == kChannelCountMono
               && isInput
        ...
        conversionNeeded = true;
        OBOE_LOGI("QuirksManager::%s() using stereo internally to avoid broken mono", __func__);
    }
