...
Result AudioOutputStreamOpenSLES::open() {
    ...
    result = OutputMixerOpenSL::getInstance().createAudioPlayer(&mObjectInterface,
                                                                          &audioSrc);
    if (SL_RESULT_SUCCESS != result) {
        OBOE_LOGE("createAudioPlayer() result:%s", getSLErrStr(result));
        goto error;
    }
    ...
    if (SL_RESULT_SUCCESS != result) {
        OBOE_LOGE("Realize player object result:%s", getSLErrStr(result));
        goto error;
    }

    result = (*mObjectInterface)->GetInterface(mObjectInterface, SL_IID_PLAY, &mPlayInterface);
    if (SL_RESULT_SUCCESS != result) {
        OBOE_LOGE("GetInterface PLAY result:%s", getSLErrStr(result));
        goto error;
    }
...
Result AudioOutputStreamOpenSLES::close() {
    OBOE_LOGD("AudioOutputStreamOpenSLES::%s()", __func__);
...
Result AudioOutputStreamOpenSLES::setPlayState_l(SLuint32 newState) {
    ...
    OBOE_LOGD("AudioOutputStreamOpenSLES(): %s() called", __func__);
    Result result = Result::OK;

    if (mPlayInterface == nullptr){
        OBOE_LOGE("AudioOutputStreamOpenSLES::%s() mPlayInterface is null", __func__);
        return Result::ErrorInvalidState;
    }

    SLresult slResult = (*mPlayInterface)->SetPlayState(mPlayInterface, newState);
    if (SL_RESULT_SUCCESS != slResult) {
        OBOE_LOGW("AudioOutputStreamOpenSLES(): %s() returned %s", __func__, getSLErrStr(slResult));
...
Result AudioOutputStreamOpenSLES::requestFlush_l() {
    ...
    if (mPlayInterface == nullptr || mSimpleBufferQueueInterface == nullptr) {
        ...
        if (slResult != SL_RESULT_SUCCESS){
            OBOE_LOGW("Failed to clear buffer queue. OpenSLES error: %d", result);
...
Result AudioOutputStreamOpenSLES::requestStop() {
    ...
    if (result == Result::OK) {
        ...
        if (requestFlush_l() != Result::OK) {
            OBOE_LOGW("Failed to flush the stream");
        }
