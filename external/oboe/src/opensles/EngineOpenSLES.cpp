...
SLresult EngineOpenSLES::open() {
    ...
    if (mOpenCount++ == 0) {
        ...
        if (SL_RESULT_SUCCESS != result) {
            OBOE_LOGE("EngineOpenSLES - slCreateEngine() result:%s", getSLErrStr(result));
            goto error;
        }
        ...
        if (SL_RESULT_SUCCESS != result) {
            OBOE_LOGE("EngineOpenSLES - Realize() engine result:%s", getSLErrStr(result));
            goto error;
        }
        ...
        if (SL_RESULT_SUCCESS != result) {
            OBOE_LOGE("EngineOpenSLES - GetInterface() engine result:%s", getSLErrStr(result));
            goto error;
        }
    }
