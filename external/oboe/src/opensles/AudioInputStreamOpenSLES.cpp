...
Result AudioInputStreamOpenSLES::open() {
    ...
    if (SL_RESULT_SUCCESS != result) {
        OBOE_LOGE("createAudioRecorder() result:%s", getSLErrStr(result));
        goto error;
    }
    ...
    if (SL_RESULT_SUCCESS != result) {
        OBOE_LOGW("%s() GetInterface(SL_IID_ANDROIDCONFIGURATION) failed with %s",
             __func__, getSLErrStr(result));
    } else {
        if (getInputPreset() == InputPreset::VoicePerformance) {
            OBOE_LOGD("OpenSL ES does not support InputPreset::VoicePerformance. Use VoiceRecognition.");
        ...
        if (SL_RESULT_SUCCESS != result
            ...
            presetValue = SL_ANDROID_RECORDING_PRESET_VOICE_RECOGNITION;
            OBOE_LOGD("Setting InputPreset %d failed. Using VoiceRecognition instead.", getInputPreset());
            mInputPreset = InputPreset::VoiceRecognition;
    ...
    if (SL_RESULT_SUCCESS != result) {
        OBOE_LOGE("Realize recorder object result:%s", getSLErrStr(result));
        goto error;
    }

    result = (*mObjectInterface)->GetInterface(mObjectInterface, SL_IID_RECORD, &mRecordInterface);
    if (SL_RESULT_SUCCESS != result) {
        OBOE_LOGE("GetInterface RECORD result:%s", getSLErrStr(result));
        goto error;
    }
...
Result AudioInputStreamOpenSLES::close() {
    OBOE_LOGD("AudioInputStreamOpenSLES::%s()", __func__);
...
Result AudioInputStreamOpenSLES::setRecordState_l(SLuint32 newState) {
    OBOE_LOGD("AudioInputStreamOpenSLES::%s(%u)", __func__, newState);
    Result result = Result::OK;

    if (mRecordInterface == nullptr) {
        OBOE_LOGE("AudioInputStreamOpenSLES::%s() mRecordInterface is null", __func__);
        return Result::ErrorInvalidState;
    }
    SLresult slResult = (*mRecordInterface)->SetRecordState(mRecordInterface, newState);
    //OBOE_LOGD("AudioInputStreamOpenSLES::%s(%u) returned %u", __func__, newState, slResult);
    if (SL_RESULT_SUCCESS != slResult) {
        OBOE_LOGE("AudioInputStreamOpenSLES::%s(%u) returned error %s",
...
Result AudioInputStreamOpenSLES::requestStart() {
    OBOE_LOGD("AudioInputStreamOpenSLES(): %s() called", __func__);
    std::lock_guard<std::mutex> lock(mLock);
...
Result AudioInputStreamOpenSLES::requestPause() {
    OBOE_LOGW("AudioInputStreamOpenSLES::%s() is intentionally not implemented for input "
         "streams", __func__);
...
Result AudioInputStreamOpenSLES::updateServiceFrameCounter() {
    ...
    if (mLock.try_lock()) {
        ...
        if (SL_RESULT_SUCCESS != slResult) {
            OBOE_LOGW("%s(): GetPosition() returned %s", __func__, getSLErrStr(slResult));
