...
Result AudioStreamOpenSLES::open() {
    ...
    OBOE_LOGI("AudioStreamOpenSLES::open() chans=%d, rate=%d", mChannelCount, mSampleRate);
...
Result AudioStreamOpenSLES::configureBufferSizes(int32_t sampleRate) {
    OBOE_LOGD("AudioStreamOpenSLES:%s(%d) initial mFramesPerBurst = %d, mFramesPerCallback = %d",
    ...
    if (mFramesPerCallback != kUnspecified) {
        ...
        if (getSdkVersion() >= __ANDROID_API_N_MR1__
            ...
            OBOE_LOGD("AudioStreamOpenSLES:%s() NOT low latency, set mFramesPerBurst = %d",
                 __func__, mFramesPerBurst);
        }
        mFramesPerCallback = mFramesPerBurst;
    }
    OBOE_LOGD("AudioStreamOpenSLES:%s(%d) final mFramesPerBurst = %d, mFramesPerCallback = %d",
         __func__, sampleRate, mFramesPerBurst, mFramesPerCallback);

    mBytesPerCallback = mFramesPerCallback * getBytesPerFrame();
    if (mBytesPerCallback <= 0) {
        OBOE_LOGE("AudioStreamOpenSLES::open() bytesPerCallback < 0 = %d, bad format?",
             mBytesPerCallback);
    ...
    if (!usingFIFO()) {
        ...
        if (mBufferCapacityInFrames <= 0) {
            ...
            OBOE_LOGE("AudioStreamOpenSLES::open() numeric overflow because mFramesPerBurst = %d",
                 mFramesPerBurst);
...
SLresult AudioStreamOpenSLES::configurePerformanceMode(SLAndroidConfigurationItf configItf) {
    ...
    if (configItf == nullptr) {
        OBOE_LOGW("%s() called with NULL configuration", __func__);
        mPerformanceMode = PerformanceMode::None;
        return SL_RESULT_INTERNAL_ERROR;
    }
    if (getSdkVersion() < __ANDROID_API_N_MR1__) {
        OBOE_LOGW("%s() not supported until N_MR1", __func__);
        mPerformanceMode = PerformanceMode::None;
    ...
    if (SL_RESULT_SUCCESS != result) {
        OBOE_LOGW("SetConfiguration(PERFORMANCE_MODE, SL %u) returned %s",
...
SLresult AudioStreamOpenSLES::updateStreamParameters(SLAndroidConfigurationItf configItf) {
    ...
    if(getSdkVersion() >= __ANDROID_API_N_MR1__ && configItf != nullptr) {
        ...
        if (SL_RESULT_SUCCESS != result) {
            OBOE_LOGW("GetConfiguration(SL_ANDROID_KEY_PERFORMANCE_MODE) returned %d", result);
...
void AudioStreamOpenSLES::processBufferCallback(SLAndroidSimpleBufferQueueItf bq) {
    ...
    if (result == DataCallbackResult::Continue) {
        ...
        if (enqueueResult != SL_RESULT_SUCCESS) {
            OBOE_LOGE("%s() returned %d", __func__, enqueueResult);
            stopStream = true;
        }
        ...
        OBOE_LOGD("Oboe callback returned Stop");
        stopStream = true;
    } else {
        OBOE_LOGW("Oboe callback returned unexpected value = %d", result);
        stopStream = true;
    }
    if (stopStream) {
        requestStop();
...
SLresult AudioStreamOpenSLES::registerBufferQueueCallback() {
    ...
    if (SL_RESULT_SUCCESS != result) {
        OBOE_LOGE("get buffer queue interface:%p result:%s",
             mSimpleBufferQueueInterface,
             getSLErrStr(result));
    } else {
        ...
        result = (*mSimpleBufferQueueInterface)->RegisterCallback(mSimpleBufferQueueInterface,
                                                                  bqCallbackGlue, this);
        if (SL_RESULT_SUCCESS != result) {
            OBOE_LOGE("RegisterCallback result:%s", getSLErrStr(result));
        }
    }
    return result;
}
