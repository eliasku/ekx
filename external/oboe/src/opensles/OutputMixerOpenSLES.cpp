...
SLresult OutputMixerOpenSL::open() {
    ...
    if (mOpenCount++ == 0) {
        ...
        if (SL_RESULT_SUCCESS != result) {
            OBOE_LOGE("OutputMixerOpenSL() - createOutputMix() result:%s", getSLErrStr(result));
            goto error;
        }
        ...
        if (SL_RESULT_SUCCESS != result) {
            OBOE_LOGE("OutputMixerOpenSL() - Realize() mOutputMixObject result:%s", getSLErrStr(result));
            goto error;
        }
    }
